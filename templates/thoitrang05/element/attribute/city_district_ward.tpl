    ...
    {if !empty($list_address)}
        {foreach from = $list_address item = address}
            ...
            {if strpos($address, '_district') > 0}
                ...
                {/if}
            {/if}
            {if strpos($address, '_ward') > 0}
                {if !empty($part_address[0])}
                    {assign var = ward_id value = $part_address[0]}
                {/if}
            {/if}
        {/foreach}
    {/if}
...
<div nh-location-wrap class="mb-5">
    ...
    <div class="form-group validate-form">
        {$this->Form->select('ward_id', $this->Location->getListWardForDropdown($district_id), ['id' => 'ward_id', 'empty' => "-- {__d('template', 'phuong_xa')} --", 'default' => $ward_id, 'class' => 'form-control form-control-sm selectpicker input-hover', 'data-size' => 10, 'data-live-search' => true])}
    </div>
