...
<div class="product-attribute-switch d-flex justify-content-start text-switch flex-wrap mb-5">
    <a href="javascript:;" nh-link-redirect="{$this->Utilities->addParamsToUrl($current_url, ["{$filter_type}{$code}" => "true"])}" nh-link-toggle="{$this->Utilities->toggleValueParamsToUrl($current_url, "{$filter_type}{$code}", "true")}" class="inner-product-attribute">
        {__d('template', 'co')}
    </a>

    <a href="javascript:;" nh-link-redirect="{$this->Utilities->addParamsToUrl($current_url, ["{$filter_type}{$code}" => "false"])}" nh-link-toggle="{$this->Utilities->toggleValueParamsToUrl($current_url, "{$filter_type}{$code}", "false")}" class="inner-product-attribute">
        {__d('template', 'khong')}
    </a>
</div>