<form id="onepay-domestic-form" action="{ADMIN_PATH}/setting/payment-gateway/{ONEPAY}" method="POST" autocomplete="off">
            ...
            {if !empty($list_language)}
                {foreach from = $list_language item = language key = k_lang}
                    <div class="form-group">
                        ...
                        <textarea id="content-onepay-{$k_lang}" name="content[{$k_lang}]" class="mce-editor-simple check-required">{if !empty($item['content'][$k_lang])}{$item['content'][$k_lang]['content']}{/if}</textarea>
                    </div>
                {/foreach}
            {/if}
        </div>
    ...
    <div class="form-group">
        ...
        <span>
            {$this->Utilities->getUrlWebsite()}/payment/webhooks/onepay
        </span>
    </div>
</form>