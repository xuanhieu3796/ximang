<form id="vnpay-form" action="{ADMIN_PATH}/setting/payment-gateway/{VNPTPAY}" method="POST" autocomplete="off">
        ...
        <div class="kt-wizard-v2__form">
            <div class="form-group row">
                ...
                <div class="col-md-6 col-12">
                    ...
                    <input name="config[encode_key]" value="{if !empty($item.config.encode_key)}{$item.config.encode_key}{/if}" type="text" class="form-control" >
                </div>
            </div>
            ...
            {if !empty($list_language)}
                {foreach from = $list_language item = language key = k_lang}
                    <div class="form-group">
                        ...
                        <textarea id="content-vnptpay-{$k_lang}" name="content[{$k_lang}]" class="mce-editor-simple check-required">{if !empty($item['content'][$k_lang])}{$item['content'][$k_lang]['content']}{/if}</textarea>
                    </div>
                {/foreach}
            {/if}
        </div>
    ...
    <div class="form-group">
        ...
        <span>
            {$this->Utilities->getUrlWebsite()}/payment/webhooks/vnptpay
        </span>
    </div>
</form>