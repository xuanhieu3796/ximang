<form id="vnpay-form" action="{ADMIN_PATH}/setting/payment-gateway/{VNPAY}" method="POST" autocomplete="off">
        ...
        <div class="kt-wizard-v2__form">
            <div class="form-group row">
                <div class="col-md-6 col-12">
                    <label>
                        {__d('admin', 'ma_website')}
                    </label>

                    <input name="config[vnp_TmnCode]" value="{if !empty($item.config.vnp_TmnCode)}{$item.config.vnp_TmnCode}{/if}" type="text" class="form-control" >
                </div>

                <div class="col-md-6 col-12">
                    <label>
                        {__d('admin', 'chuoi_tao_checksum')}
                    </label>

                    <input name="config[vnp_HashSecret]" value="{if !empty($item.config.vnp_HashSecret)}{$item.config.vnp_HashSecret}{/if}" type="text" class="form-control">
                </div>
            </div>
            ...
            {if !empty($list_language)}
                {foreach from = $list_language item = language key = k_lang}
                    <div class="form-group">
                        ...
                        <textarea id="content-vnpay-{$k_lang}" name="content[{$k_lang}]" class="mce-editor-simple check-required">{if !empty($item['content'][$k_lang])}{$item['content'][$k_lang]['content']}{/if}</textarea>
                    </div>
                {/foreach}
            {/if}
        </div>
    ...
    <div class="form-group">
        ...
        <span>
            {$this->Utilities->getUrlWebsite()}/payment/webhooks/vnpay
        </span>
    </div>
