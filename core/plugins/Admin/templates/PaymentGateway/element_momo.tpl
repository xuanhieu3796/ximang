<form id="momo-form" action="{ADMIN_PATH}/setting/payment-gateway/{MOMO}" method="POST" autocomplete="off">
        ...
        <div class="kt-wizard-v2__form">
            <div class="form-group row">
                <div class="col-md-6 col-12">
                    ...
                    <input name="config[partner_code]" value="{if !empty($item.config.partner_code)}{$item.config.partner_code}{/if}" type="text" class="form-control" >
                </div>
                ...
                <div class="col-md-6 col-12">
                    ...
                    <input name="config[ios_scheme]" value="{if !empty($item.config.ios_scheme)}{$item.config.ios_scheme}{/if}" type="text" class="form-control" >
                    <span class="form-text text-muted">
                        Sử dụng khi thanh toán APP IN APP
                    </span>
                </div>
            </div>
            ...
            {if !empty($list_language)}
                {foreach from = $list_language item = language key = k_lang}
                    <div class="form-group">
                        ...
                        <textarea id="content-momo-{$k_lang}" name="content[{$k_lang}]" class="mce-editor-simple check-required">{if !empty($item['content'][$k_lang])}{$item['content'][$k_lang]['content']}{/if}</textarea>
                    </div>
                {/foreach}
            {/if}
        </div>
    ...
    <div class="form-group">
        ...
        <span>
            {$this->Utilities->getUrlWebsite()}/payment/webhooks/momo
        </span>
    </div>
</form>