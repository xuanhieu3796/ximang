...
<div list-comment="{PRODUCT}" data-type="{PRODUCT_DETAIL}">
	<div class="row">
		<div class="col-lg-6">
				...
				<div class="kt-portlet__body kt-portlet__body--fit">
					<div class="kt-widget kt-widget--project-1 p-0 m-0 border rounded">
						...
						<div class="kt-widget__body">
							<div nh-comment="{if !empty($id)}{$id}{/if}" class="kt-notes comment-section1 mt-15">
							    <ul nh-list-comment="" class="list-comment1 kt-notes__items">
							    	{if empty($number_comment)}
							    		<i>{__d('admin', 'san_pham_chua_co_binh_luan_nao')}</i>
							    	{/if}
							    </ul>
							</div>
						</div>
						...
						<div class="kt-widget__body">
							<div nh-rating="{if !empty($id)}{$id}{/if}" class="mt-15">
							    <ul nh-list-rating="" class="list-rating">
							    	{if empty($number_rating)}
							    		<i>{__d('admin', 'san_pham_chua_co_danh_gia_nao')}</i>
							    	{/if}
							    </ul>
							</div>
						</div>
