<div class="row">
    ...
    <div class="col-lg-3 col-12">
        <div class="form-group">
            ...
            {$this->Form->select("config[{SORT_FIELD}]", $this->TemplateAdmin->getListSortFieldOfRating(), ['empty' => "{__d('admin', 'chon')}", 'default' => "{if !empty($config[{SORT_FIELD}])}{$config[{SORT_FIELD}]}{/if}", 'class' => 'form-control form-control-sm kt-selectpicker'])}
        </div>
    </div>
...
<input type="hidden" name="config[block_code]" value="{if !empty($code)}{$code}{/if}">