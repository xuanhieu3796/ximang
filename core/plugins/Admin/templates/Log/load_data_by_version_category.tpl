...
<table class="table mb-0">
	...
	{/foreach}
		...
		{if !empty($before_change.parent_id)}
			{$main_category_before = $this->CategoryAdmin->getAllNameContent($before_change.parent_id)}
		{/if}
		{assign var = main_category_after value = ''}
		{if !empty($after_change.parent_id)}
			{$main_category_after = $this->CategoryAdmin->getAllNameContent($after_change.parent_id)}
		{/if}
		...
		{if !empty($before_change.attributes)}
			{$attributes_before = $this->AttributeAdmin->formatDataAttributes($before_change.attributes , $lang_log, 'category')}
		{/if}
		{assign var = attributes_after value = []}
		{if !empty($after_change.attributes)}
			{$attributes_after = $this->AttributeAdmin->formatDataAttributes($after_change.attributes , $lang_log, 'category')}
		{/if}
