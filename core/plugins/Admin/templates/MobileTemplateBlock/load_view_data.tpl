{if $data_type == {CATEGORY_PRODUCT} || $data_type == {CATEGORY_ARTICLE}}
    <div class="form-group position-relative">
        ...
        {if !empty($list_categories)}
            <div class="col-md-6 col-12 kt-scroll" data-scroll="true" data-height="274" data-scrollbar-shown="true">
                {foreach from = $list_categories key = category_id item = category}
                    ...
                    <div class="clearfix mt-10">
                        ...
                        <label class="kt-checkbox kt-checkbox--tick kt-checkbox--success mb-5">
                            <input name="data_ids[]" value="{$category_id}" {if !empty($data) && $category_id|in_array:$data}checked="true"{/if} type="checkbox"> 
...
{assign var = filter value = false}
{if $data_type == {CATEGORY_PRODUCT}}
    {$filter = true}
{/if}

{if $data_type == {BY_PAGE_ID} && !empty($block_type) && $block_type == {PRODUCT}}
    {$filter = true}
{/if}
    ...
    <div class="row">
        <div class="col-lg-3 col-12">
            <div class="form-group">
                ...
                {$this->Form->select('filter_data', $list_filter, ['empty' => {__d('admin', 'tat_ca')}, 'default' => "{if !empty($config_data.filter_data)}{$config_data.filter_data}{/if}", 'class' => 'form-control form-control-sm kt-selectpicker'])}
            </div>
        </div>
    </div>
{/if}