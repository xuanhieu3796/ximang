        ...
        <table class="table table-striped- table-bordered table-hover table-checkable dataTable no-footer dtr-inline">
            <thead class="thead-light">
                <tr>
                    ...
                    <th rowspan="2" data-field="staff" 
                        data-sort="{if !empty($data_sort) && $data_sort eq 'asc' && $data_field eq 'staff'}desc{else}asc{/if}" 
                        class="{if !empty($data_field) && $data_field eq 'staff'}sorted{/if}">
                        {__d('admin', 'nhan_vien')}
                    </th>
