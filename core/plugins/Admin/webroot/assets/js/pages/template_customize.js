...
var nhTemplateCustomize = function () {
	var formEl;
    ...
    var nhPageConfig = {
    	...
    	init: function(){
			...
			$(document).on('click', '.btn-update-page', function(e) {
				...
				nhMain.callAjax({
					url: adminPath + '/template/page/get-type',
					data: {
						code: pageCode
					},
				}).done(function(response) {
		            ...
		            if (code == _SUCCESS) {
		            	var pageType = typeof(data.page_type) ? data.page_type : null;		            	
		            	switch(pageType) {
						  	case _PAGE:
						  		$(self.idModal).modal('show');
    							$(self.idModal).find(self.idForm).html('');
						    ...
						    break;
						  	...
						  	case _LAYOUT:
						  		$(self.idModalLayout).modal('show');
    							$(self.idModalLayout).find(self.idFormLayout).html('');
						    	self.loadViewLayout({code: pageCode});
						    break;
						}
		            }
				});
			...
			// event for UI create layout --------------------------------
			$(document).on('click', '.btn-create-layout', function(e) {
				$(self.idModalLayout).modal('show');
    			$(self.idModalLayout).find(self.idFormLayout).html('');
				...
				nhMain.callAjax({
					url: adminPath + '/template/page/load-info-layout',
					data: data,
					dataType: 'html'
				}).done(function(response) {
					$(self.idModalLayout + ' ' + self.idFormLayout).html(response);
					self.validate();
				   	KTApp.unblockPage();
				});
			});

			$(document).on('click', '#btn-save-layout', function(e) {
    			e.preventDefault();
    			...
    			if (self.validator.form()) {
					...
					var formData = $(self.idFormLayout).serialize();
					nhMain.callAjax({
						url: $(self.idFormLayout).attr('action'),
						data: formData
					}).done(function(response) {
						KTApp.unprogress(btnSave);
			...
			$(document).on('click', '.btn-delete-config-page', function(e) {
				...
				swal.fire({
			    	...
			    	if(typeof(result.value) != _UNDEFINED && result.value){
			    		nhMain.callAjax({
							url: adminPath + '/template/page/delete-config',
							data:{
								code: code,
								device: device
							}
			...
			$(document).on('click', '.btn-delete-page', function(e) {
				...
				swal.fire({
			    	...
			    	if(typeof(result.value) != _UNDEFINED && result.value){
			    		nhMain.callAjax({
							url: adminPath + '/template/page/delete',
							data:{
								code: code
							}
						}).done(function(response) {
			...
			//event UI dulicate page ---------------------------------------
			$(document).on('click', '.btn-dulicate-page', function(e) {
				var code = $('#select-page').val();


				swal.fire({
			        title: nhMain.getLabel('nhan_ban_trang'),
			        // text: nhMain.getLabel('ban_co_chac_chan_muon_nhan_ban_trang_nay'),
			        type: 'info',
			        
			        confirmButtonText: `<i class="fa fa-copy"></i> ${nhMain.getLabel('dong_y')}`,
			        ...
			        input: 'checkbox',
					inputValue: 1,
					inputPlaceholder: nhMain.getLabel('nhan_ban_cac_block_trong_trang')
			    }).then(function(result) {
			    	...
			    	nhMain.callAjax({
						url: adminPath + '/template/page/duplicate',
						data:{
							code: code,
							allow_duplicate_block: result.value || 0
						}
					}).done(function(response) {
						var code = response.code || _ERROR;
					    var message = response.message || '';
					    var data = response.data || {};
		        		var pageCode = data.code || '';
    		...
    		$(document).on('click', '.btn-select-device', function(e) {
    			if(!$(this).hasClass('active')){
    				...
    				formEl.find('#tab-log').attr('data-device', device);
    	...
    	loadView: function(params = {}){
			...
			nhMain.callAjax({
				async: true,
				url: adminPath + '/template/page/load-info',
				data: data,
				dataType: 'html'
			}).done(function(response) {				
    	...
    	loadViewLayout: function(params = {}){
    		var self = this;

    		KTApp.blockPage(blockOptions);
    		var data = {
    			code: typeof(params.code) != _UNDEFINED ? params.code : null,
    			language: typeof(params.language) != _UNDEFINED ? params.language : null,
    		};
			nhMain.callAjax({
				url: adminPath + '/template/page/load-info-layout',
				data: data,
    	...
    	loadDropdownCategory: function(type = null){
    		...
    		nhMain.callAjax({
    			async: true,
				url: adminPath + '/template/page/load-dropdown-category/' + type,
				dataType: 'html'
			}).done(function(response) {
    	...
    	toggleLink: function(type = null){
    		...
    		if(type == _NORMAL || type == _PRODUCT || type == _ARTICLE || type == _PRODUCT_DETAIL || type == _ARTICLE_DETAIL || type == _HOME){
    	...
    	loadDropdownPage: function(pageCode = null){
    		...
    		nhMain.callAjax({
    			async: true,
				url: adminPath + '/template/page/load-dropdown-page',
				data:{
					code: pageCode
				},
				dataType: 'html'
    ...
    var nhThemeConfig = {
    	...
    	init: function(){
			...
			$(document).on('change', '#select-page', function(e) {
				...
				if($('[nh-tab="log-tab"]').length > 0){
					$('[nh-tab="log-tab"]').attr('record-code', $(this).val());
				}
			});
    	...
    	getConfig: function(){
    		...
    		$(self.idWrap).find('.wrap-list-item').each(function(i) {
    			var type = $(this).data('type-row');    			
    			if(typeof(result[type]) == _UNDEFINED){
    				return;
    			}
    			...
    			$(this).find(self.classRowItem + ':not(.disable)').each(function(i) {
    				...
    				var dataRow = [];
    				var dataColumn = [];
    				var codeRow = typeof($(this).data('code')) != _UNDEFINED && $(this).data('code').length > 0 ? $(this).data('code') : null;
    				var configRow = typeof($(this).attr('data-config')) != _UNDEFINED && $(this).attr('data-config').length > 0 ? $(this).attr('data-config') : null;
    				...
    				_row.find(self.classColumnItem).each(function(i) {
    					...
    					var blocks = [];
    					_column.find('.block-item').each(function(i) {
    						var code = typeof($(this).data('code')) != _UNDEFINED ? $(this).data('code') : null;
    						if(code.length > 0){
    							blocks.push($(this).data('code'));	
    						}    						
    					});
    	...
    	loadStructureTemplate: function(page_code = null){
    		...
    		nhMain.callAjax({
    			async: true,
    			dataType: 'html',
				url: adminPath + '/template/page/load-structure',
				data: {
					code: page_code,
					device: device
				}
