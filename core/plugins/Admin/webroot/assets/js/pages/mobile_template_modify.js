...
var nhMobileTemplateModify = function () {
	var ktTree = {
		ext: null,
		path: null,
		...
		init: function() {
	        ...
	        self.load_folder.on(
		        "rename_node.jstree", function(evt, data){
		            ...
		            nhMain.callAjax({
			            url: adminPath + '/mobile-app/modify/rename-file',
			            type: 'POST',
			            data: {
			            	new_name: data.text,
			            	old_name: data.old,
		        ...
		        'delete_node.jstree', function(evt, data){
		        	...
		        	swal.fire({
				    	...
				    	if(typeof(result.value) != _UNDEFINED && result.value){
				    		nhMain.callAjax({
					            url: adminPath + '/mobile-app/modify/delete-file',
					            type: 'POST',
					            data: {
					            	path: path,
					            	type: type
		...
		readFolder: function() {
			...
			nhMain.callAjax({
                url: adminPath + '/mobile-app/modify/read-folder',
            }).done(function(response) {
                var code = typeof(response.code) != _UNDEFINED ? response.code : _ERROR;
                var message = typeof(response.message) != _UNDEFINED ? response.message : '';
                var data = typeof(response.data) != _UNDEFINED ? response.data : '';
		...
		fileContextMenu: function($node, tree){
			return {
		        ...
		        'download': {
		            ...
		            action: function (obj) {
		            	window.location.href = adminPath + '/mobile-app/modify/download-file?path=' + $node.original.a_attr.data_file_path;
                    }
		        },
		        'rename': {
		...
		loadFile: function(_path, _ext) {
		    ...
		    nhMain.callAjax({
	            url: adminPath + '/mobile-app/modify/load-file',
	            type: 'POST',
	            data: {
	            	path: _path
	            }
		...
		saveFile: function() {			
			...
			var editor = ace.edit('editor-template');
			nhMain.callAjax({
                url: adminPath + '/mobile-app/modify/save-file',
                data: {
	...
	return {
		init: function() {		 
			...
			nhMain.selectMedia.dropzoneUpload({
				id: 'uploadFile',
                url: adminPath + '/mobile-app/modify/upload-file',
