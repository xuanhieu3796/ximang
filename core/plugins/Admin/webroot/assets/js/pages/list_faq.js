...
var nhListFaq = function() {
	var options = {
		data: {
			type: 'remote',
			source: {
				read: {
					url: adminPath + '/faq/list/json',
					headers: {
		...
		columns: [
			...
			},
			{
				...
				template: function(row) {
					...
					var urlEdit = adminPath + '/faq/update/' + row.id;
	...
	return {
		listData: function() {
		    ...
		    nhList.eventDefault(datatable, {
		    	url: {
			    	delete: adminPath + '/faq/delete',
			    	status: adminPath + '/faq/change-status',
			    	quickChange: adminPath + '/faq/change-position'
			    }
		    });		
