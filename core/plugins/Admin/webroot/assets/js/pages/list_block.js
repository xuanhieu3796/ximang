...
var nhListBlock = function() {
	var options = {
		data: {
			type: 'remote',
			source: {
				read: {
					url: adminPath + '/template/block/list/json',
					headers: {
		...
		columns: [
			...
			},
			{
				...
				template: function(row) {
					...
					var urlEdit = adminPath + '/template/block/update/' + row.code;
					return '\
	...
	return {
		listData: function() {
		    ...
		    nhList.eventDefault(datatable, {
		    	url: {
			    	delete: adminPath + '/template/block/delete',
			    	status: adminPath + '/template/block/change-status',
			    	duplicate: adminPath + '/template/block/duplicate',
			    }
		    });
